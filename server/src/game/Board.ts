...
export class Board {
  ...
  private static getPieceFromBag(seed: number, bag: PieceType[]): PieceTypeRng {
  ...
  private static handleFilledRows(grid: number[][]): clearRowsData {
    const nonCompleteRows = grid.filter((row) =>
      row.some(
        (cell) => Board.isEmptyCell(cell) || cell === GRID_STATES.BLOCKED
      )
    );
  ...
  private static isValidCoordinates(
    ...
    return coordinates.every(
      ({ x, y }) =>
        Board.isCoordinateInBound({ x, y }) && Board.isEmptyCell(grid[y][x])
    );
  }
  ...
  static handleGameInput(
    ...
    if (newInput === GameInput.DOWN) {
      return Board.handleDownInput(grid, piece, boardData.seed, boardData.bag);
    }
  ...
  private static computeGhost(piece: Piece, grid: number[][]): Piece {
    ...
    while (true) {
      ...
      const isValid = Piece.getComputedCoordinates(next).every(
        ({ x, y }) =>
          this.isCoordinateInBound({ x, y }) && grid[y][x] === GRID_STATES.EMPTY
      );
