      ...
      <div className="bg-gray-800/70 border-t border-b border-electric-red/50 w-full max-w-sm px-6 py-8 sm:px-8 sm:py-10 rounded-xl backdrop-blur-sm">
        {gameOverOverlay.ranking[0]?.name === playerName ? (
          ...
          <div className="flex flex-col items-center animate-pulse">
            ...
            <p className="text-xl font-medium text-slate-300 tracking-wide">
              Score :{" "}
              {
                gameOverOverlay.ranking.find((val) => val.name === playerName)
                  ?.points
              }
            </p>
            <p className="text-xl font-medium text-slate-300 tracking-wide">
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="mt-5 grid grid-cols-1 gap-2 w-full py-3">
            {Array.from(
              {
                length: Math.max(0, gameOverOverlay.ranking.length - 1),
              },
              (_, index) => (
                <div
                  key={index}
                ...
                >
                  <span>
                    {index + 2 + " - "}
                    {gameOverOverlay.ranking[index + 1]?.name ?? "Empty"}
                  </span>
                  <span>
                    {gameOverOverlay.ranking[index + 1]?.points ?? "0"}
                  </span>
                </div>
              ),
            )}
